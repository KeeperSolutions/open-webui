				...
				>
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
						<div class="flex flex-col gap-0.5">
							<label for="featured-model-provider-{idx}" class="text-xs text-gray-500"
								>{$i18n.t('Provider Name')} <span class="text-red-500">*</span></label
							>
							<input
								id="featured-model-provider-{idx}"
								type="text"
								required
								minlength={PROVIDER_NAME_MIN}
								maxlength={PROVIDER_NAME_MAX}
								aria-invalid={!isProviderNameValid(featuredModels[idx].provider_name)}
								class="text-sm rounded-lg px-2.5 py-1.5 bg-gray-50 dark:bg-gray-900 border focus:outline-none focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-600 w-full {isProviderNameValid(
									featuredModels[idx].provider_name
								)
							...
							/>
							{#if !isProviderNameValid(featuredModels[idx].provider_name)}
								<span class="text-[11px] text-red-500">
									{$i18n.t('{{min}}–{{max}} characters', {
										min: PROVIDER_NAME_MIN,
						...
						<div class="flex flex-col gap-0.5">
							<label for="featured-model-name-{idx}" class="text-xs text-gray-500"
								>{$i18n.t('Featured Name')}</label
							>
							<input
					...
					<div class="grid grid-cols-3 gap-2">
						{#each [0, 1, 2] as tagIdx}
							<div class="flex flex-col gap-0.5">
								<label for="featured-model-tag-{idx}-{tagIdx}" class="text-xs text-gray-500"
									>{$i18n.t('Tag {{n}}', { n: tagIdx + 1 })}</label
								>
								<input
