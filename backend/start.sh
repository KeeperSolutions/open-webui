...
if [[ "${ENABLE_DB_MIGRATIONS,,}" == "true" ]]; then
  # set -e is disabled around this command so a non-zero exit is caught by
  # migration_status below instead of aborting the script before the
  # diagnostic message (and the deliberate non-zero `exit`) can run.
  set +e
  "$PYTHON_CMD" <<'PY'
