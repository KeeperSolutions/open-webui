...
AIOHTTP_CLIENT_TIMEOUT_MODEL_LIST = os.getenv(
    'AIOHTTP_CLIENT_TIMEOUT_MODEL_LIST',
    os.getenv('AIOHTTP_CLIENT_TIMEOUT_OPENAI_MODEL_LIST', '10'),
)
...
_sock_read_env = os.getenv('AIOHTTP_CLIENT_TIMEOUT_SOCK_READ')
...
LLM_RETRY_MAX = os.getenv("LLM_RETRY_MAX", "3")
try:
...
LLM_RETRY_MAX_DURATION_SECONDS = os.getenv("LLM_RETRY_MAX_DURATION_SECONDS", "90")
try:
...
_llm_retry_status_env = os.getenv("LLM_RETRY_RETRYABLE_STATUS", "")
...
AIOHTTP_CLIENT_TIMEOUT_TOOL_SERVER_DATA = os.getenv('AIOHTTP_CLIENT_TIMEOUT_TOOL_SERVER_DATA', '10')
...
STRIPE_FREE_TIER_CENTS = int(
    os.getenv('STRIPE_FREE_TIER_CENTS', '200')
)  # €2.00 trial credit
BILLING_GRACE_PERIOD_DAYS = int(os.getenv('BILLING_GRACE_PERIOD_DAYS', '3'))
USAGE_POLL_INTERVAL_SECONDS = int(os.getenv('USAGE_POLL_INTERVAL_SECONDS', '120'))  # Default 2 minutes

# Comma-separated list of email domains considered internal (e.g. "keepersolutions.com")
INTERNAL_EMAIL_DOMAINS = [
    d.strip().lower()
    for d in os.getenv('INTERNAL_EMAIL_DOMAINS', '').split(',')
    if d.strip()
]
...
UNLIMITED_USER_EMAILS: set[str] = {
    e.strip().lower()
    for e in os.getenv('UNLIMITED_USER_EMAILS', '').split(',')
    if e.strip()
}
TRIAL_CREDIT_EUR = float(os.getenv('TRIAL_CREDIT_EUR', '2.0'))
...
try:
    LEDGER_BOOTSTRAP_DAYS = int(os.getenv('LEDGER_BOOTSTRAP_DAYS', '30'))
except ValueError:
...
SMTP_FROM_NAME: str = os.getenv('SMTP_FROM_NAME', 'Hubgate')
