...
spec 01 creates accounts that spec 02 logs into.
...
**Purpose:** post-upgrade smoke test of the core promise of the product —
sign in, pick a model, chat, sign out — for both a privileged and an
unprivileged account, plus the setup that spec 02 depends on.

**Prerequisites it establishes for spec 02:** an approved
`role: 'user'` account (`E2E_USER_EMAIL` / `TEST_PASSWORD`) and a model
shared publicly so non-admins can select it.
...
| 3 | Open the user menu | A **"Playground"** entry is present | Positive control for step 9's permission-boundary check |
| 4 | Type a prompt, Send | User message echoed; the `{#if message.done}`-gated action row (`[aria-label="Edit"]`) appears within 120 s | Real LLM round-trip; the "conversation stuck after an error" bug (an unset `done` looks identical from the UI); Ollama 400 on tool-incapable models (the retry-without-tools fallback) |
...
| 9 | Open the user menu | **No** "Playground" entry | Role actually took — admin-only UI is gated |
| 10 | Select the shared model, send a prompt | User message echoed; response completes (`[aria-label="Edit"]` visible) | The non-privileged chat path works end to end; model access grant is readable |
...
**Role:** the **regular (non-admin) user** created by spec 01.
**Purpose:** go past "a message round-trips" into the areas the
`v0.9.6 → v0.11.0` merge actually churned — persistence, reload,
history, regenerate — on the **unprivileged** path, where a merge can
break them (or a per-role chat permission) while the admin path and
every backend unit test stay green.
...
| 2 | Select the (shared) model, send a prompt | User message echoed; response completes; URL becomes `/c/<uuid>` | Chat is **saved on first message**; `message.done` gating |
| 3 | **Full page reload** | Same `/c/<uuid>` URL; **both** turns still rendered; action row present | The `chat` blob **save + load round-trip**, and `EncryptedJSONField` **re-decrypt** (`ENC1:`, TRAU-434) — all of which pass backend unit tests even when broken |
| 4 | Look at the sidebar (expanded — `primeAppState` sets `localStorage.sidebar='true'`) | `#sidebar-new-chat-button` present; ≥1 `a[href^="/c/"]` history row | History list renders |
...
| 7 | Click **Regenerate** → **"Try Again"** in the dropdown (`$settings.regenerateMenu` defaults on) | The prev/next-message pager (`[aria-label="Previous message"]`) appears within 120 s; the regenerated turn completes | `chat.regenerate_response` per-role permission; response **branching** (sibling versions); the `process_chat_response` / `message.done` area |
